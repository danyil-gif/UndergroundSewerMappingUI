# SIMD Underground Sewer Mapping Prototype — Rebuild Document

This document contains everything needed to recreate this prototype exactly. Copy each file verbatim.
...
- Fonts: DM Sans + JetBrains Mono (Google Fonts, wired in `src/index.css`)
...
pnpm install
pnpm dev        # starts on port 8443 (or $PORT)
```
...
<html lang="en">
  <head>
    ...
    <title>SIMD Sewer Mapping</title>
  </head>
  <body>
    <div id="root"></div>
...
interface CamVideo {
  ...
  runStart?: { footage: string; pipeType: string; pipeSize: string }
  runEnd?: { footage: string; pipeType: string; pipeSize: string }
...
const SAMPLE_PIPES: Pipe[] = [
  ...
  },
  {
    ...
    waypoints: [], diameter: '10"', length: "89", depth: "8.2", material: "HDPE", slope: "0.8%",
    videos: []
  },
  {
    id: "p3", label: "PIPE-003", fromId: "a5", toId: "a3",
    waypoints: [], diameter: '8"', length: "73", depth: "7.1", material: "RCP", slope: "1.5%",
    videos: []
  },
  {
    id: "p4", label: "PIPE-004", fromId: "a4", toId: "a5",
    waypoints: [], diameter: '6"', length: "54", depth: "4.8", material: "PVC", slope: "2.1%",
    videos: []
  },
]
...
function AssetNode({ asset, selected, drawActive, scale, onClick, onMouseDown }: {
  asset: Asset; selected: boolean; drawActive: boolean; scale: number
  onClick: () => void; onMouseDown: (e: React.MouseEvent) => void
}) {
  const c = drawActive ? "#F59E0B" : C.cyan
  return (
    <div
      style={{
        ...
        filter: selected ? `drop-shadow(0 0 6px ${c}88)` : undefined,
...
const inputSt: React.CSSProperties = {
  ...
  border: "1px solid #D2DAE2", borderRadius: 4, color: "#16202A",
...
function FieldLabel({ text, pricing }: { text: string; pricing?: boolean }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 5 }}>
      ...
      {pricing && <span style={{ fontSize: 8, fontWeight: 600, color: "#A96B00", border: "1px solid #A96B0055", borderRadius: 3, padding: "0px 4px", letterSpacing: "0.06em" }}>PRICING</span>}
    </div>
  )
}

function FootageRow({ footage, onChange }: { footage: string; onChange: (v: string) => void }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <div style={{ fontSize: 9, fontWeight: 600, color: "#5F6E7C", letterSpacing: "0.09em", textTransform: "uppercase", whiteSpace: "nowrap" }}>FOOTAGE *</div>
      <input type="number" value={footage} onChange={e => onChange(e.target.value)} style={{ ...inputSt, width: 60, textAlign: "center" }} />
      {[0, 5, 10, 25].map(d => (
        <button key={d} onClick={() => onChange(String((parseFloat(footage || "0") + d).toFixed(0)))} style={{ padding: "5px 7px", fontSize: 9, fontWeight: 600, background: "#fff", color: "#5F6E7C", border: "1px solid #D2DAE2", borderRadius: 3, cursor: "pointer", fontFamily: "JetBrains Mono" }}>+{d}</button>
      ))}
    </div>
  )
}
...
function FormActions({ onSave, onCancel, disabled }: { onSave: () => void; onCancel: () => void; disabled?: boolean }) {
  return (
    <div style={{ display: "flex", gap: 7, marginTop: 2 }}>
      ...
      <button onClick={onSave} disabled={!!disabled} style={{ flex: 2, padding: "8px", fontSize: 10.5, fontWeight: 700, letterSpacing: "0.05em", background: disabled ? "#EDF1F4" : "#16202A", color: disabled ? "#94A3B8" : "#fff", border: "none", borderRadius: 5, cursor: disabled ? "default" : "pointer" }}>
        Save
      </button>
    </div>
  )
...
export default function App() {
  ...
  const [pipeForm, setPipeForm] = useState<Partial<Pipe>>({})
  ...
  const [runEndForm, setRunEndForm] = useState({ footage: "", pipeType: "PVC", pipeSize: '4"' })
  ...
  const selectedVideo = selectedPipe?.videos.find(v => v.id === selectedVideoId)
  const assetPipes = selectedAsset ? pipes.filter(p => p.fromId === selectedAsset.id || p.toId === selectedAsset.id) : []

  // ── Map interaction ──────────────────────────────────────────────────────────

  // Convert container-space coords → content-space coords (reversing the zoom transform)
  const toContentCoords = useCallback((clientX: number, clientY: number): { x: number; y: number } => {
    if (!mapRef.current) return { x: 0, y: 0 }
    const rect = mapRef.current.getBoundingClientRect()
    // Reverse the interactive pan/zoom transform first
    const normX = (clientX - rect.left) / rect.width
    const normY = (clientY - rect.top) / rect.height
    const rawX = ((normX - mapPan.x - 0.5) / mapZoom + 0.5) * 100
    const rawY = ((normY - mapPan.y - 0.5) / mapZoom + 0.5) * 100
  ...
  const handleMapMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    // Middle mouse, alt+drag, or left-click-hold when zoomed in → pan
    if (e.button === 1 || (e.button === 0 && mode === "view" && (e.altKey || mapZoom > 1))) {
      e.preventDefault()
      setIsPanning(true)
  ...
  }, [mode, toContentCoords, mapPan])

  const completePipe = useCallback(() => {
    if (!drawFrom || drawPoints.length === 0) return
    ...
    const newPipe: Pipe = {
      ...
      waypoints,
      diameter: '8"', length: "", depth: "", material: "PVC", slope: "",
      videos: [],
    }
    ...
    setEditingPipe(true)
    setPipeForm({ diameter: '8"', material: "PVC", length: "", depth: "", slope: "" })
  }, [drawFrom, drawPoints, pipes])

  const handleMapClick = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    if (dragId || mode === "select-area") return
    if (!mapRef.current) return
    // double-click while drawing = complete
    if (mode === "draw-pipe" && drawFrom && e.detail === 2) {
      completePipe()
      return
    }
    const { x, y } = toContentCoords(e.clientX, e.clientY)
    ...
    // draw-pipe: add a free-point routing waypoint
    if (mode === "draw-pipe" && drawFrom) {
      setDrawPoints(prev => [...prev, { x, y }])
    }
  }, [mode, addAssetType, assets, dragId, drawFrom, completePipe, toContentCoords])
  ...
  }, [dragId, selectStart, toContentCoords, mode, drawFrom, isPanning, panStart])
  ...
  const savePipe = () => {
    ...
    setPipes(prev => prev.map(p => p.id === selectedPipeId ? { ...p, ...pipeForm } : p))
    setEditingPipe(false)
  }

  const addVideo = () => {
    if (!selectedPipeId || !videoForm.name) return
    const vid: CamVideo = {
      id: `v${Date.now()}`, name: videoForm.name, date: videoForm.date,
      operator: videoForm.operator, direction: videoForm.direction, observationsClosed: false, observations: [],
    }
  ...
  const saveObservation = () => {
    ...
    const obs: Observation = { id: `obs${Date.now()}`, ...captureForm } as Observation
  ...
  const saveRunEnd = () => {
    if (!selectedPipeId || !selectedVideoId || !runEndForm.footage) return
    setPipes(prev => prev.map(p =>
      p.id === selectedPipeId
        ? { ...p, videos: p.videos.map(v => v.id === selectedVideoId ? { ...v, runEnd: { footage: runEndForm.footage, pipeType: runEndForm.pipeType, pipeSize: runEndForm.pipeSize } } : v) }
        : p
    ))
    setShowRunEndForm(false)
    setRunEndForm({ footage: "", pipeType: "PVC", pipeSize: '4"' })
  }
  ...
  const selectPipe = (id: string) => {
    ...
    setEditingPipe(false); setMode("view")
  }
  ...
  const saveMapView = () => {
    ...
    const newView: MapViewRect = { id: `mv${Date.now()}`, name: newViewName.trim(), ...pendingView }
  ...
  const selectedViewPipes = selectedView
    ...
    : []
  const zoomScale = activeView ? 100 / activeView.w : 1
  const zoomTransform = activeView
    ? `scale(${zoomScale}) translate(-${activeView.x}%, -${activeView.y}%)`
    : undefined
  ...
  return (
    <div
      style={{ height: "100%", display: "flex", overflow: "hidden", background: C.bg, color: C.text, fontFamily: "'DM Sans', sans-serif", userSelect: "none" }}
      onMouseMove={handleMouseMove}
      onMouseUp={handleMouseUp}
    >

      {/* ── LEFT PANEL ─────────────────────────────────────────────────────────── */}
      <div style={{ width: 252, minWidth: 252, background: C.panel, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        {/* Add infrastructure */}
        <Section>
          <Label>Add Infrastructure</Label>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, marginBottom: 6 }}>
            {ALL_ASSET_TYPES.map(t => {
              ...
              return (
                <button
                  key={t}
                  onClick={() => { setAddAssetType(t); setMode("add-asset"); setDrawFrom(null) }}
                  style={{
          ...
          <button
            onClick={() => { setMode(mode === "draw-pipe" ? "view" : "draw-pipe"); setDrawFrom(null) }}
            style={{
          ...
          >
            ...
            {mode === "draw-pipe"
              ? (drawFrom ? `${drawPoints.length} pts — Complete or keep clicking` : "Click an asset to start…")
              : "Draw Pipe"}
          </button>
        </Section>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "10px 0" }}>
          <div style={{ padding: "0 16px 6px", fontSize: 9, fontWeight: 600, color: C.muted, letterSpacing: "0.08em", textTransform: "uppercase" }}>
        ...
        {/* Pipe list */}
        <div style={{ borderTop: `1px solid ${C.border}`, padding: "10px 0", maxHeight: 160, overflowY: "auto" }}>
          <div style={{ padding: "0 16px 6px", fontSize: 9, fontWeight: 600, color: C.muted, letterSpacing: "0.08em", textTransform: "uppercase" }}>
            Pipes ({pipes.length})
          </div>
          {pipes.map(pipe => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: sel ? "#E0F0FA" : "transparent",
                  borderLeft: `2px solid ${sel ? C.blue : needsAttention ? "#A96B00" : "transparent"}`,
                  transition: "background 0.1s",
                  display: "flex", alignItems: "center", gap: 8,
                }}
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: sel ? "#0369A1" : C.muted, fontFamily: "JetBrains Mono" }}>{pipe.label}</div>
                  <div style={{ fontSize: 10, color: C.muted }}>{fr?.label ?? "?"} → {to?.label ?? "free"} · ⌀{pipe.diameter}</div>
                </div>
                {analysisComplete && (
                ...
                {needsAttention && !analysisComplete && (
                  <div title="Analysis incomplete — needs attention" style={{ flexShrink: 0, width: 16, height: 16, borderRadius: "50%", background: "#A96B00", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontWeight: 700, color: "#fff", fontFamily: "JetBrains Mono" }}>!</div>
                )}
              </div>
            )
          })}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", position: "relative" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <span style={{ fontSize: 10, color: C.muted, fontFamily: "JetBrains Mono" }}>
              ...
              {mode === "draw-pipe" && !drawFrom && "DRAW — click an asset to start the route"}
              {mode === "draw-pipe" && drawFrom && !drawPoints.length && `DRAW — click map to add points  ·  click asset to snap  ·  double-click or Complete to finish`}
              {mode === "draw-pipe" && drawFrom && drawPoints.length > 0 && `DRAW — ${drawPoints.length} point${drawPoints.length > 1 ? "s" : ""}  ·  keep clicking or double-click to finish`}
              {mode === "select-area" && "FRAME — drag to define a map view area"}
            </span>
          </div>
          {mode !== "view" && (
            <div style={{ display: "flex", gap: 6 }}>
              {mode === "draw-pipe" && drawFrom && drawPoints.length > 0 && (
                <button
                  onClick={completePipe}
                  style={{ padding: "3px 12px", fontSize: 9.5, fontWeight: 700, background: C.cyan, color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", letterSpacing: "0.05em" }}
                >
                  ✓ Complete Pipe
                </button>
              )}
              <button onClick={() => { setMode("view"); setDrawFrom(null); setDrawPoints([]); setDrawMouse(null); setSelectStart(null); setSelectCurrent(null) }} style={{ padding: "3px 9px", fontSize: 9.5, background: C.card, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 4, cursor: "pointer", fontFamily: "JetBrains Mono", letterSpacing: "0.05em" }}>
                ESC / CANCEL
              </button>
            </div>
          )}
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              {showIconScaleDropdown && (
                <div onMouseDown={e => e.stopPropagation()} style={{ position: "absolute", top: "calc(100% + 4px)", right: 0, zIndex: 100, background: C.card, border: `1px solid ${C.border}`, borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,0.10)", padding: "10px 14px", minWidth: 220, display: "flex", flexDirection: "column", gap: 10 }}>
                  <div style={{ fontSize: 9, fontWeight: 700, color: C.dim, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 2 }}>ICON SIZE BY TYPE</div>
                  {ALL_ASSET_TYPES.map(t => {
                    const label = ASSET_META[t].label
                    const v = iconScaleByType[t]
                    return (
                      <div key={t}>
                        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 10.5, color: C.text, fontWeight: 500 }}>
                          <span>{label}</span>
                          <span style={{ fontFamily: "JetBrains Mono", fontSize: 9.5, color: C.muted }}>{Math.round(v * 100)}%</span>
                        </div>
                        <input type="range" min="0.5" max="2.5" step="0.05" value={v}
                  ...
                  <button onClick={() => setIconScaleByType(Object.fromEntries(ALL_ASSET_TYPES.map(t => [t, 1])) as Record<AssetType, number>)} style={{ marginTop: 2, padding: "5px", fontSize: 9.5, background: C.panel, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 4, cursor: "pointer" }}>Reset all</button>
                </div>
              )}
            </div>
        ...
        <div style={{
            ...
            return (
              <button
                ...
                style={{
                  ...
                  height: 36, border: "none", borderBottom: `2px solid ${active ? C.cyan : "transparent"}`,
          ...
          {mapViews.map(view => {
            ...
            return (
              ...
              >
                <button
                  onClick={() => { setActiveTabId(view.id); setSelectedViewId(view.id); setSelectedAssetId(null); setSelectedPipeId(null) }}
                  style={{
                    display: "flex", alignItems: "center", gap: 6, padding: "0 12px 0 14px",
                    height: 36, border: "none", background: "transparent",
                ...
                <button
                  ...
                  title="Delete view"
                  style={{
                    width: 24, height: 36, border: "none", background: "transparent",
        ...
        >
          ...
          <div style={{
            ...
            cursor: isPanning ? "grabbing" : (mode === "view" && mapZoom > 1) ? "grab" : undefined,
          ...
          {mapImage && <div style={{ position: "absolute", inset: 0, background: "rgba(240,244,248,0.25)" }} />}
          ...
          <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", overflow: "visible" }}>
            ...
            {pipes.map(pipe => {
              ...
              const toX = toAsset ? toAsset.x : (pipe.toX ?? fr.x)
              const toY = toAsset ? toAsset.y : (pipe.toY ?? fr.y)
              ...
              const lineColor = sel ? C.blue : hov ? "#0369A1" : hasVid ? C.cyan : "#B0C4D8"
                ...
                <g key={pipe.id}>
                  ...
                  {pts.slice(0, -1).map((pt, i) => (
                    <line key={i}
                      ...
                      onClick={e => { e.stopPropagation(); selectPipe(pipe.id) }}
                  ...
                  {pts.slice(0, -1).map((pt, i) => (
                    <line key={`v${i}`}
                      x1={`${pt.x}%`} y1={`${pt.y}%`}
                      x2={`${pts[i + 1].x}%`} y2={`${pts[i + 1].y}%`}
                      stroke={lineColor} strokeWidth={sel ? 2.5 : 1.8}
                  ...
                  {(hov || sel) && (
                    <g style={{ pointerEvents: "none" }}>
                      ...
                      <text x={`${midPt.x}%`} y={`${midPt.y}%`} textAnchor="middle" fill={lineColor} fontSize="8.5" fontFamily="JetBrains Mono">
                        ...
                        <tspan x={`${midPt.x}%`} dy="12" fill="#64748B">⌀{pipe.diameter}{pipe.material ? ` ${pipe.material}` : ""}</tspan>
                      </text>
                    </g>
                  )}
                  {!hov && !sel && (
                    <text x={`${midPt.x}%`} y={`${midPt.y}%`} textAnchor="middle" dy="-7"
                      fill={hasVid ? C.cyan : "#B0C4D8"} fontSize="8" fontFamily="JetBrains Mono"
                      style={{ pointerEvents: "none" }}>
                      {pipe.label}
                    </text>
                  )}
            ...
            {mode === "draw-pipe" && drawFrom && (() => {
              const startAsset = assets.find(a => a.id === drawFrom)
              if (!startAsset) return null
              const preview: RoutePoint[] = [{ x: startAsset.x, y: startAsset.y }, ...drawPoints]
                ...
                <g style={{ pointerEvents: "none" }}>
                  ...
                  {drawPoints.map((pt, i) => (
                    <circle key={i} cx={`${pt.x}%`} cy={`${pt.y}%`} r="4"
                      fill={pt.assetId ? C.cyan : C.panel} stroke={C.cyan} strokeWidth="1.8"
                    />
                  ))}
                  ...
                  {/* Start node ring */}
                  <circle cx={`${startAsset.x}%`} cy={`${startAsset.y}%`} r="16"
                  ...
                  {drawPoints.length > 0 && (
                    <>
                      <rect x={`calc(${lastPt.x}% + 10px)`} y={`calc(${lastPt.y}% - 10px)`}
                        width="38" height="16" rx="3" fill={C.cyan} />
          ...
          {/* Legend */}
          <div style={{
            position: "absolute", bottom: 16, left: 16, background: C.panel + "EE",
            border: `1px solid ${C.border}`, borderRadius: 6, padding: "10px 14px",
            display: "flex", flexDirection: "column", gap: 5,
          }}>
            {[
              { color: C.cyan, label: "Inspected pipe", dash: false },
              { color: "#B0C4D8", label: "Uninspected pipe", dash: true },
            ].map(({ color, label, dash }) => (
              <div key={label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <svg width="28" height="8">
                  <line x1="0" y1="4" x2="28" y2="4" stroke={color} strokeWidth="2" strokeDasharray={dash ? "6 4" : undefined} />
                </svg>
                <span style={{ fontSize: 9, color: C.muted, fontFamily: "JetBrains Mono" }}>{label}</span>
              </div>
            ))}
          </div>
          ...
          {activeView && (
            <div style={{
              ...
              {activeView.name}
              <button onClick={() => setActiveTabId("main")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.75)", cursor: "pointer", padding: 0, fontSize: 13, lineHeight: 1 }}>×</button>
            </div>
          )}
        </div>
      </div>
      ...
      {/* ── RIGHT PANEL ───────────────────────────────────────────────────────── */}
      <div style={{ width: panelW, minWidth: panelW, background: C.panel, borderLeft: panelOpen ? `1px solid ${C.border}` : "none", display: "flex", flexDirection: "column", overflow: "hidden", transition: "width 0.25s cubic-bezier(0.4,0,0.2,1), min-width 0.25s cubic-bezier(0.4,0,0.2,1)" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0, marginRight: 8 }}>
                    ...
                    {editingViewId === view.id ? (
                      <div style={{ display: "flex", gap: 6, marginTop: 2 }}>
                        ...
                        />
                        <button onClick={() => saveViewName(view.id)} style={{ padding: "5px 10px", background: C.cyan, color: "#fff", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 700, cursor: "pointer" }}>Save</button>
                ...
                <Section>
                  ...
                  {viewPipes.map(p => (
                    <div key={p.id} onClick={() => selectPipe(p.id)} style={{ padding: "8px 10px", marginBottom: 5, borderRadius: 5, background: C.card, border: `1px solid ${C.border}`, cursor: "pointer" }}>
                      ...
                      <div style={{ fontSize: 10, color: C.muted }}>⌀{p.diameter} · {p.material} · {p.videos.length} inspection{p.videos.length !== 1 ? "s" : ""}</div>
                    </div>
                  ))}
                </Section>
              )}
        ...
        {selectedAsset && !selectedPipeId && (
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
            <Section>
              ...
              {editingLocationId === selectedAsset.id ? (
                <div style={{ marginTop: 8, padding: "8px 10px", borderRadius: 6, background: "#EFF8FF", border: `1px solid ${C.cyan}44`, display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button onClick={() => { setEditingLocationId(null); setLockedAssetIds(prev => new Set([...prev, selectedAsset.id])) }} style={{ flexShrink: 0, padding: "4px 10px", fontSize: 9.5, fontWeight: 700, background: C.cyan, color: "#fff", border: "none", borderRadius: 4, cursor: "pointer" }}>Confirm</button>
                </div>
              ) : (
            ...
            {["cleanout-floor","cleanout-stack","cleanout-foundation"].includes(selectedAsset.type) && (
              <Section>
            ...
            {selectedAsset.type === "gutter-hub" && (
              <Section>
                ...
                <div style={{ display: "flex", gap: 6, marginBottom: selectedAsset.cameraAccessible ? 10 : 0 }}>
                  {[true, false].map(v => {
                    const sel = selectedAsset.cameraAccessible === v
                    return <button key={String(v)} onClick={() => setAssets(prev => prev.map(a => a.id === selectedAsset.id ? { ...a, cameraAccessible: v } : a))} style={{ flex: 1, padding: "8px", fontSize: 11, fontWeight: 700, borderRadius: 5, cursor: "pointer", background: sel ? C.cyan : "#fff", color: sel ? "#fff" : C.muted, border: `1.5px solid ${sel ? C.cyan : C.border}` }}>{v ? "YES" : "NO"}</button>
                  })}
                </div>
                {selectedAsset.cameraAccessible && (
                  <button style={{ width: "100%", padding: "8px", fontSize: 10.5, fontWeight: 700, background: C.card, color: C.cyan, border: `1.5px solid ${C.cyan}`, borderRadius: 5, cursor: "pointer", letterSpacing: "0.04em" }}>+ ADD SEWER CAMERA</button>
                )}
              </Section>
            )}
                ...
                : assetPipes.map(pipe => (
                  <div key={pipe.id} onClick={() => selectPipe(pipe.id)} style={{ padding: "9px 12px", marginBottom: 5, borderRadius: 6, cursor: "pointer", background: C.card, border: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>⌀{pipe.diameter} · {pipe.length ? pipe.length + " ft" : "—"} · {pipe.material || "—"}</div>
                    </div>
        ...
        {selectedPipe && (
          <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: C.muted, marginTop: 2 }}>
                    {assets.find(a => a.id === selectedPipe.fromId)?.label} → {selectedPipe.toId ? assets.find(a => a.id === selectedPipe.toId)?.label : "free endpoint"}
                  </div>
                </div>
                <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                  <button
                    onClick={() => {
                      ...
                      setEditingPipe(true)
                      setPipeForm({ diameter: selectedPipe.diameter, length: selectedPipe.length, depth: selectedPipe.depth, material: selectedPipe.material, slope: selectedPipe.slope })
                    }}
              ...
              {editingPipe ? (
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                    {[
                      { key: "diameter", label: "Diameter", placeholder: '8"' },
                      { key: "material", label: "Material", placeholder: "PVC, VCP, HDPE…" },
                      { key: "length", label: "Length (ft)", placeholder: "0.0" },
                      { key: "depth", label: "Depth (ft)", placeholder: "0.0" },
                      { key: "slope", label: "Slope (%)", placeholder: "0.00%" },
                    ].map(({ key, label, placeholder }) => (
                      <InputField
                        key={key} label={label}
                        value={(pipeForm as Record<string, string>)[key] || ""}
                        onChange={v => setPipeForm(f => ({ ...f, [key]: v }))}
                        placeholder={placeholder}
                      />
                    ))}
                  </div>
                  <button
                    onClick={savePipe}
                    style={{
                      marginTop: 2, padding: "8px", background: C.blue, color: "#fff",
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
                  {[
                    { label: "Diameter", value: selectedPipe.diameter || "—" },
                    { label: "Material", value: selectedPipe.material || "—" },
                    { label: "Length", value: selectedPipe.length ? selectedPipe.length + " ft" : "—" },
                    { label: "Depth", value: selectedPipe.depth ? selectedPipe.depth + " ft" : "—" },
                    { label: "Slope", value: selectedPipe.slope || "—" },
                  ].map(({ label, value }) => (
            ...
            <Section>
              ...
              {showVideoForm && (
                <div style={{ padding: 12, background: C.card, borderRadius: 6, border: `1px solid ${C.border}`, marginBottom: 10, display: "flex", flexDirection: "column", gap: 6 }}>
                  <InputField label="Filename" value={videoForm.name} onChange={v => setVideoForm(f => ({ ...f, name: v }))} placeholder="inspection.mp4" />
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                    <InputField label="Date" value={videoForm.date} onChange={v => setVideoForm(f => ({ ...f, date: v }))} placeholder="YYYY-MM-DD" />
                    <InputField label="Operator" value={videoForm.operator} onChange={v => setVideoForm(f => ({ ...f, operator: v }))} placeholder="Name" />
                  </div>
                  <div>
                    <div style={{ fontSize: 9, color: C.muted, marginBottom: 3, textTransform: "uppercase", letterSpacing: "0.07em" }}>Direction</div>
                    <select value={videoForm.direction} onChange={e => setVideoForm(f => ({ ...f, direction: e.target.value as "upstream" | "downstream" }))} style={{ width: "100%", padding: "6px 8px", background: C.panel, border: `1px solid ${C.border}`, borderRadius: 4, color: C.text, fontSize: 11, outline: "none" }}>
                      <option value="downstream">Downstream</option>
                      <option value="upstream">Upstream</option>
                    </select>
                  </div>
                  <button onClick={addVideo} style={{ padding: "8px", background: "#00803E", color: "#fff", border: "none", borderRadius: 4, fontSize: 11, fontWeight: 700, cursor: "pointer", letterSpacing: "0.05em" }}>
                    ADD TO ANALYSES
                  </button>
                </div>
              )}
                    ...
                    <div style={{ padding: "10px 12px", display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div style={{ display: "flex", gap: 5, flexShrink: 0 }}>
                        <button
                          ...
                          style={{ padding: "3px 9px", fontSize: 9.5, fontWeight: 700, background: isOpen ? C.blue : "#00803E", color: "#fff", border: "none", borderRadius: 3, cursor: "pointer", letterSpacing: "0.04em" }}
                        >
                          {isOpen ? "CLOSE" : "OPEN"}
                        </button>
              ...
              <div style={{ borderTop: `2px solid ${C.border}` }}>
                ...
                <div style={{ padding: "12px 16px 10px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button
                    ...
                    style={{ flexShrink: 0, padding: "5px 10px", fontSize: 9.5, fontWeight: 700, letterSpacing: "0.05em", background: "#16202A", color: "#fff", border: "none", borderRadius: 5, cursor: "pointer", display: "flex", alignItems: "center", gap: 5 }}
                  >
                    <svg width="11" height="11" viewBox="0 0 11 11" fill="none">
                ...
                <div style={{ padding: "12px 16px 0" }}>
                  <div style={{ width: "100%", aspectRatio: "16/9", background: "#0A1520", borderRadius: 8, border: `1px solid ${C.border}`, position: "relative", overflow: "hidden", marginBottom: 8 }}>
                    <svg width="100%" height="100%" viewBox="0 0 328 185" preserveAspectRatio="xMidYMid slice">
                      ...
                      <text x="244" y="176" fill="#94A3B8" fontSize="7.5" fontFamily="JetBrains Mono">⌀{selectedPipe?.diameter}</text>
                    </svg>
                    {/* Timeline scrubber */}
                  ...
                  <div style={{ margin: "0 16px 12px", borderRadius: 8, border: "1.5px solid #A96B00", background: "#FFFBF5", overflow: "hidden" }}>
                    ...
                    {captureStep === "tie-in" && (
                      <div style={{ padding: "14px 14px 12px", display: "flex", flexDirection: "column", gap: 10 }}>
                        ...
                        <FootageRow footage={String(captureForm.footage ?? "")} onChange={v => setCaptureForm(f => ({ ...f, footage: v }))} />
                ...
                {/* Region D — run length */}
                <div style={{ padding: "12px 16px 0" }}>
                  <div style={{ fontSize: 9, fontWeight: 600, color: C.muted, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>RUN LENGTH</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    ...
                    <div style={{ padding: "10px 11px", borderRadius: 6, border: `1px solid ${C.border}`, background: C.card }}>
                      ...
                      {selectedVideo.runStart ? (
                        <>
                          <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "JetBrains Mono", color: "#16202A" }}>{selectedVideo.runStart.footage} ft</div>
                    ...
                    >
                      ...
                      {selectedVideo.runEnd ? (
                        <>
                          <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "JetBrains Mono", color: "#16202A" }}>{selectedVideo.runEnd.footage} ft</div>
                  ...
                  {selectedVideo.runEnd && selectedVideo.runStart && (
                    <div style={{ marginTop: 8, padding: "7px 10px", borderRadius: 5, background: "#E8F4EF", border: "1px solid #00803E44", textAlign: "center", fontFamily: "JetBrains Mono", fontSize: 12, fontWeight: 700, color: "#00803E" }}>
                      {Math.abs(parseFloat(selectedVideo.runEnd.footage) - parseFloat(selectedVideo.runStart.footage)).toFixed(0)} ft inspected
                    </div>
                  )}
                  ...
                  {showRunEndForm && (
                    <div style={{ marginTop: 10, padding: "12px 12px 10px", borderRadius: 8, border: "1.5px solid #CE1A74", background: "#FFF8FA", display: "flex", flexDirection: "column", gap: 8 }}>
                      ...
                      <div style={{ padding: "7px 9px", borderRadius: 4, background: "#F0F4F8", fontSize: 9.5, color: C.muted, lineHeight: 1.45 }}>
                        This becomes the inspected footage for the run and closes the last pipe section.
                      </div>
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <button onClick={saveRunEnd} disabled={!runEndForm.footage || !runEndForm.pipeType || !runEndForm.pipeSize} style={{ flex: 2, padding: "7px", fontSize: 10, fontWeight: 700, background: runEndForm.footage ? "#CE1A74" : C.card, color: runEndForm.footage ? "#fff" : C.dim, border: "none", borderRadius: 5, cursor: runEndForm.footage ? "pointer" : "default" }}>
                          Save Run End
                        </button>
                      </div>
                    </div>
                  ...
                  {selectedVideo.observations.length === 0 ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                      {[...selectedVideo.observations].sort((a, b) => parseFloat(a.footage || "0") - parseFloat(b.footage || "0")).map(obs => {
                        ...
                        return (
                          <div key={obs.id} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "8px 10px", borderRadius: 6, background: C.card, border: `1px solid ${col}18`, borderLeft: `3px solid ${col}` }}>
                            {/* Footage */}
                            <div style={{ flexShrink: 0, minWidth: 34, textAlign: "right" }}>
                              <div style={{ fontSize: 11, fontFamily: "JetBrains Mono", fontWeight: 700, color: col }}>{obs.footage ?? "—"}</div>
      ...
      {inspectionFullscreen && selectedVideo && selectedPipe && (() => {
        ...
        const videoSvg = (
          <svg width="100%" height="100%" viewBox="0 0 640 360" preserveAspectRatio="xMidYMid slice">
            ...
            <text x="470" y="340" fill="#94A3B8" fontSize="11.5" fontFamily="JetBrains Mono">⌀{pipe.diameter} {pipe.material}</text>
          </svg>
        )
        ...
        const obsRows = [...vid.observations].sort((a, b) => parseFloat(a.footage || "0") - parseFloat(b.footage || "0")).map(obs => {
          ...
          return (
            <div key={obs.id} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "10px 12px", borderRadius: 7, background: "#fff", border: `1px solid ${col}18`, borderLeft: `3px solid ${col}` }}>
              <div style={{ flexShrink: 0, minWidth: 38, textAlign: "right" }}>
                <div style={{ fontSize: 13, fontFamily: "JetBrains Mono", fontWeight: 700, color: col }}>{obs.footage ?? "—"}</div>
            ...
            <div style={{ flex: "0 0 28%", display: "flex", flexDirection: "column", background: "#fff", minHeight: 0 }}>
              ...
              <div style={{ flexShrink: 0, padding: "10px 14px 9px", borderBottom: "1px solid #E8EDF2", display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <button
                  ...
                  style={{ flexShrink: 0, padding: "8px 12px", fontSize: 9.5, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", background: (videoPlaying || vid.observationsClosed) ? "#E2E8F0" : "#00803E", color: (videoPlaying || vid.observationsClosed) ? "#94A3B8" : "#fff", border: "none", borderRadius: 5, cursor: (videoPlaying || vid.observationsClosed) ? "default" : "pointer", transition: "all 0.15s" }}
                >
                  Capture Observation
                </button>
                {/* Exit button */}
                <button onClick={() => setInspectionFullscreen(false)} title="Exit full screen" style={{ flexShrink: 0, width: 28, height: 28, display: "flex", alignItems: "center", justifyContent: "center", background: "#F1F5F9", border: "none", borderRadius: 5, cursor: "pointer", color: "#5F6E7C" }}>
              ...
              <div style={{ flex: 1, overflowY: "auto", minHeight: 0 }}>
                ...
                {captureStep !== "none" && (
                  <div style={{ borderBottom: "1px solid #E8EDF2", background: "#FFFBF5", borderLeft: "3px solid #A96B00" }}>
                    {formPanel}
                  </div>
                )}
                ...
                <div style={{ padding: "12px 14px", borderBottom: "1px solid #E8EDF2" }}>
                  <div style={{ fontSize: 8.5, fontWeight: 700, color: "#5F6E7C", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>RUN LENGTH</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    <div style={{ padding: "10px 12px", borderRadius: 6, border: "1px solid #D2DAE2", background: "#F8FAFC" }}>
                      ...
                      {vid.runStart
                        ? <><div style={{ fontSize: 18, fontWeight: 700, fontFamily: "JetBrains Mono", color: "#16202A", lineHeight: 1.1 }}>{vid.runStart.footage} ft</div><div style={{ fontSize: 9.5, color: "#5F6E7C", marginTop: 3 }}>{vid.runStart.pipeSize} {vid.runStart.pipeType}</div></>
                    ...
                    <div onClick={() => { if (vid.observationsClosed && !vid.runEnd) setShowRunEndForm(true) }}
                      ...
                      {vid.runEnd
                        ? <><div style={{ fontSize: 18, fontWeight: 700, fontFamily: "JetBrains Mono", color: "#16202A", lineHeight: 1.1 }}>{vid.runEnd.footage} ft</div><div style={{ fontSize: 9.5, color: "#5F6E7C", marginTop: 3 }}>{vid.runEnd.pipeSize} {vid.runEnd.pipeType}</div></>
                  ...
                  {vid.runEnd && vid.runStart && (
                    <div style={{ marginTop: 8, padding: "6px 10px", borderRadius: 5, background: "#E8F4EF", border: "1px solid #00803E44", textAlign: "center", fontFamily: "JetBrains Mono", fontSize: 12, fontWeight: 700, color: "#00803E" }}>
                      {Math.abs(parseFloat(vid.runEnd.footage) - parseFloat(vid.runStart.footage)).toFixed(0)} ft inspected
                    </div>
                  )}
                  {showRunEndForm && (
                    <div style={{ marginTop: 10, padding: "12px 12px 10px", borderRadius: 7, border: "1.5px solid #CE1A74", background: "#FFF8FA", display: "flex", flexDirection: "column", gap: 8 }}>
                      ...
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <button onClick={saveRunEnd} disabled={!runEndForm.footage} style={{ flex: 2, padding: "7px", fontSize: 10, fontWeight: 700, background: runEndForm.footage ? "#CE1A74" : "#F8FAFC", color: runEndForm.footage ? "#fff" : "#94A3B8", border: "none", borderRadius: 4, cursor: runEndForm.footage ? "pointer" : "default" }}>Save Run End</button>
                      </div>
                    </div>
                  )}
              ...
              <div style={{ flexShrink: 0, borderTop: "1px solid #E8EDF2" }}>
                <button
                  ...
                  style={{ display: "block", width: "100%", padding: "13px 14px", fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", cursor: "pointer", background: vid.observationsClosed ? "#F1F5F9" : "#16202A", color: vid.observationsClosed ? "#5F6E7C" : "#fff", border: "none", borderBottom: "1px solid #E8EDF2", transition: "all 0.15s" }}
                >
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setIncompletePipeIds(prev => { const s = new Set(prev); s.delete(pipe.id); return s })
                    setInspectionFullscreen(false)
                    setSelectedVideoId(null)
                    setCaptureStep("none")
                  }}
                  style={{ display: "block", width: "100%", padding: "13px 14px", fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", border: "none", cursor: (vid.observationsClosed && vid.runEnd) ? "pointer" : "default", background: (vid.observationsClosed && vid.runEnd) ? "#00803E" : "#EDF1F4", color: (vid.observationsClosed && vid.runEnd) ? "#fff" : "#94A3B8" }}
                >
                  Complete Analysis
                </button>
                {!vid.observationsClosed && (
                  <button
                    onClick={() => {
                      setIncompletePipeIds(prev => new Set([...prev, pipe.id]))
                      setInspectionFullscreen(false)
                      setSelectedVideoId(null)
                      setCaptureStep("none")
                    }}
                    style={{ display: "block", width: "100%", padding: "9px 14px", fontSize: 10, fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase", background: "transparent", color: "#A96B00", border: "none", borderTop: "1px solid #E8EDF2", cursor: "pointer" }}
                  >
      ...
      {/* ── PENDING VIEW NAME DIALOG ──────────────────────────────────────────── */}
      {pendingView && (
        <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ position: "absolute", inset: 0, background: "rgba(15,25,35,0.45)", backdropFilter: "blur(2px)" }} onClick={() => setPendingView(null)} />
          <div style={{
            position: "relative", background: C.panel, border: `1px solid ${C.border}`,
            borderRadius: 10, padding: 24, width: 340, boxShadow: "0 20px 60px rgba(0,0,0,0.18)",
          }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 4 }}>Name this map view</div>
            <div style={{ fontSize: 11, color: C.muted, marginBottom: 14 }}>
              Area: {pendingView.w.toFixed(1)}% × {pendingView.h.toFixed(1)}% · captures {assets.filter(a => a.x >= pendingView.x && a.x <= pendingView.x + pendingView.w && a.y >= pendingView.y && a.y <= pendingView.y + pendingView.h).length} assets
            </div>
            <input
              autoFocus
              value={newViewName}
              onChange={e => setNewViewName(e.target.value)}
              onKeyDown={e => { if (e.key === "Enter") saveMapView(); if (e.key === "Escape") setPendingView(null) }}
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button onClick={() => setPendingView(null)} style={{ padding: "7px 16px", background: C.card, color: C.muted, border: `1px solid ${C.border}`, borderRadius: 6, fontSize: 12, cursor: "pointer" }}>
                Cancel
              </button>
              <button onClick={saveMapView} style={{ padding: "7px 18px", background: C.cyan, color: "#fff", border: "none", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
                Save View
              </button>
            </div>
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              ...
              <button
                ...
                style={{ padding: "7px 18px", background: "#DC2626", color: "#fff", border: "none", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer" }}
              >
                Confirm Delete
              </button>
            </div>
...
### Layout
- Full-height two-column shell: left sidebar (260px) + map area (flex-1) + collapsible right panel
- Right panel collapses to width=0 when nothing is selected, expanding the map area
- Transition: `width 0.25s ease`

### Left Sidebar Sections
1. **Header** — "SIMD" logo + project name
2. **Mode toolbar** — View / Add Infrastructure / Draw Pipe / Select Area toggle buttons
3. **Map Views** — thumbnail tabs for Main + custom cropped views; "Add View" button
4. **Pipes** — list of pipe segments with green ✓ (complete) or amber ! (attention needed) badges
5. **Infrastructure Assets** — list with color-coded type chips

### Map Area
- Background image fills 100% (SVG or `<img>` tag)
- Pan/zoom with `transformOrigin: 50% 50%`
  - Mouse wheel zooms pivoting on cursor position
  - Pinch-to-zoom for tablet (`touchstart`/`touchmove` with distance calc)
  - Click-hold to pan when `mapZoom > 1`
  - +/− buttons with % readout overlay
- Asset icons: shape-based SVG (circle/square/diamond/triangle/hexagon) + abbreviation
- Green ✓ badge on map for pipes with completed analysis
- Per-type icon size sliders in dropdown

### Asset System (13 types)
| Type | Abbr | Shape | Group |
|------|------|-------|-------|
| catch-basin | CB | diamond | basin |
| storm-basin | SB | diamond | basin |
| sanitary-basin | SAB | diamond | basin |
| cleanout-floor | CF | square | cleanout |
| cleanout-stack | CS | square | cleanout |
| cleanout-foundation | CW | square | cleanout |
| cleanout-overhead | CO | square | cleanout |
| stack-no-cleanout | SK | triangle | stack |
| floor-drain | FD | circle | drain |
| gutter-hub | GH | hexagon | drain |
| turf-drain | TD | circle | drain |
| ejector-pump | EP | hexagon | pump |
| sump-pump | SP | hexagon | pump |

**Asset location locking**: assets are locked by default. "Edit Location" button in detail panel unlocks drag; "Confirm Location" re-locks.

### Pipe Drawing
- Click map to add routing waypoints; snap to existing assets
- Second endpoint is optional
- Polyline rendered with SVG; midpoint label chip

### Camera / Observation Fullscreen
- Clicking a video in the pipe panel opens fullscreen overlay
- Left 72%: full-bleed video player (SVG placeholder) with scrubber pinned at bottom
- Right 28%: three zones
  - **Pinned top**: status indicator + "Capture Observation" button + × close
  - **Scrollable middle**: active observation form / run-length input / observation log
  - **Pinned bottom**: "Done Adding Observations" + "Complete Analysis" buttons

### Coordinate System Math (critical)
Transform: `transformOrigin: 50% 50%`

Forward (content → screen normalized):
```
sx = (cx - 0.5) * zoom + panX + 0.5
```

Inverse (screen → content normalized):
```
cx = (sx - panX - 0.5) / zoom + 0.5
```
Then multiply by 100 for percentage coordinates.

Zoom pivot (keep point `ox,oy` fixed):
```
panX_new = ox - 0.5 - (ox - 0.5 - panX_old) * ratio
```

### Known Gotchas
- IIFE with `return (` inside JSX causes Vite parse error — compute variables before the component's `return` statement
- After large rewrites, hard-refresh browser (Ctrl+Shift+R) to clear stale module cache
- `toContentCoords` must subtract `0.5` before dividing by zoom (see inverse formula above)
